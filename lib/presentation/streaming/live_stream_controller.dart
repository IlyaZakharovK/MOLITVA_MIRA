...
final liveTranslationRepositoryProvider = Provider<LiveTranslationRepository>((ref) {
...
class LiveStreamState {
  ...
  LiveStreamState copyWith({
    ...
    return LiveStreamState(
      ...
      needsUserGestureToPlay: needsUserGestureToPlay ?? this.needsUserGestureToPlay,
      ...
      prayerTextError: clearPrayerTextError ? null : (prayerTextError ?? this.prayerTextError),
    );
  }
}

final liveStreamControllerProvider =
StateNotifierProvider.autoDispose.family<LiveStreamController, LiveStreamState, int>(
      (ref, translationId) => LiveStreamController(ref, translationId: translationId),
);

class LiveStreamController extends StateNotifier<LiveStreamState> {
  LiveStreamController(this.ref, {required this.translationId}) : super(LiveStreamState.initial()) {
  ...
  LiveTranslationRepository get _repo => ref.read(liveTranslationRepositoryProvider);
  AuthLocalStore get _local => ref.read(authLocalStoreProvider);
  PrayerRequestRepository get _prayersRepo => ref.read(prayerRequestRepositoryProvider);
  ...
  Future<void> _pingPresence({required int userId, required int translationId}) async {
    if (!mounted) return;
    ...
    try {
      ...
      debugPrint('[LIVE][PRESENCE] ok count=${info.countOnline} users=${info.users.length}');
  ...
  Future<void> _pollVad() async {
    ...
    try {
      ...
      if (audioLevel != null) {
        ...
        if (_lastTotalAudioEnergy != null && _lastTotalSamplesDuration != null) {
      ...
      final bool speaking = speakingNow || now.difference(_lastSpeakingAt).inMilliseconds < 450;

      if (!mounted) return;
      final lvlOut = _smoothedLevel.clamp(0.0, 1.0);
      if (state.speaking != speaking || (state.speakingLevel - lvlOut).abs() > 0.03) {
  ...
  Future<void> _resolvePrayerText(LiveTranslation tr) async {
    ...
    try {
      final prayers = await _prayersRepo.getPrayersByCategory(tr.prayers_category_id);
      ...
      final textToShow = (resolved != null && resolved.trim().isNotEmpty) ? resolved : tr.description;
  ...
  Future<void> _init() async {
    try {
      state = state.copyWith(isLoading: true, clearError: true, status: 'Загрузка трансляции…');
      ...
      final canSpeakByRole =
          role == ProfileRole.clergy || role == ProfileRole.temple || role == ProfileRole.admin;

      final tr = await _repo.fetchById(translationId);

      // ✅ Подмена понятия isOwner = "может говорить"
      final isOwner = (localUserId != null && localUserId == tr.ownerId) || canSpeakByRole;
      ...
      if (!mounted) return;
      state = state.copyWith(isLoading: false, error: e.toString(), status: 'Ошибка: $e');
    }
  }
  ...
  Future<void> _connectWsAndJoin({required LiveTranslation tr, required bool isOwner}) async {
    _log('WS: подключение…');
    ...
    _wsSub = _ws!.stream.listen((msg) async {
      ...
      if (msg.containsKey('status') && !msg.containsKey('type')) {
        ...
        if (!joinCompleter.isCompleted) joinCompleter.completeError(Exception(m));
        return;
      }
      ...
      if ((msg['type'] ?? '').toString() == 'iceMtr' && _mtrPc != null) {
        ...
        if (cand is Map) {
          ...
          if (candidate.isNotEmpty) {
            try {
              await _mtrPc!.addCandidate(RTCIceCandidate(candidate, sdpMid, idx));
            } catch (_) {}
          }
        }
        return;
  ...
  // ===== Main RTC =====
  Future<void> _setupWebRtcAndNegotiate({required LiveTranslation tr, required bool isOwner}) async {
  ...
  // ===== Metronome RTC =====
  Future<void> _setupMetronomeIfNeeded({required LiveTranslation tr, required String offerSdp}) async {
    ...
    await _mtrPc!.setRemoteDescription(RTCSessionDescription(offerSdp, 'offer'));
    final answer = await _mtrPc!.createAnswer({'offerToReceiveAudio': 1, 'offerToReceiveVideo': 0});
  ...
  Future<void> toggleSound() async {
    ...
    if (next == AudioOutputMode.muted) {
      ...
      if (state.metronomeOn && _lastOfferMtrSdp != null && state.translation != null) {
        await _setupMetronomeIfNeeded(tr: state.translation!, offerSdp: _lastOfferMtrSdp!);
      }
    }
